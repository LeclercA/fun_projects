...
while ($j < $grid_columns) {
    ...
    while ($i < $len) {
        ...
        $isVertical = false;
        if ($i < $len - 3) {
            $allValues[] = $grid[$j][$i] * $grid[$j][$i + 1] * $grid[$j][$i + 2] * $grid[$j][$i + 3];
            $isHorizontal = true;
            //right top
            if ($j >= 3) {
                $allValues[] = $grid[$j][$i] * $grid[$j - 1][$i + 1] * $grid[$j - 2][$i + 2] * $grid[$j - 3][$i + 3];
            }
        }
        if ($j < $grid_columns - 3) {
            $allValues[] = $grid[$j][$i] * $grid[$j + 1][$i] * $grid[$j + 2][$i] * $grid[$j + 3][$i];
            $isVertical = true;
        }
        //right bottom
